...
class EngineStatus:
    def __init__(self):
        ...
        self.STATUS_TG = f"Pyro v2.0.106"
        self.STATUS_YT = f"yt-dlp v2023.3.4"
        ...
        self.STATUS_RCLONE = f"Ftp {version_cache['rclone']}"
...
def get_readable_message():
    ...
    for download in list(download_dict.values())[STATUS_START:STATUS_LIMIT+STATUS_START]:
        ...
        if download.status() not in [MirrorStatus.STATUS_SPLITTING, MirrorStatus.STATUS_SEEDING]:
            msg += BotTheme('BAR', Bar=f"[<a href='{download.message.link}'>{get_progress_bar_string(download.progress())}</a>] {download.progress()}")
    ...
    #buttons.ubutton(BotTheme('BOOST'), f"https://t.me/SachinMirror/41", 'header')
    ...
    actual_free_space_bytes = disk_usage(config_dict['DOWNLOAD_DIR']).free
    actual_total_space_bytes = disk_usage(config_dict['DOWNLOAD_DIR']).total

    modified_free_space_bytes = actual_free_space_bytes + 70 * 1024 * 1024 * 1024
    modified_total_space_bytes = actual_total_space_bytes + 95 * 1024 * 1024 * 1024

    modified_free_space_str = get_readable_file_size(modified_free_space_bytes)

    free_percentage = 100 * modified_free_space_bytes / modified_total_space_bytes

    memory      = virtual_memory()
    dummy_total_memory = memory.total - 21 * 1024 * 1024 * 1024
    dummy_available_memory = memory.available - 19 * 1024 * 1024 * 1024
    dummy_used_memory = dummy_total_memory - dummy_available_memory
    mem_percent = round((dummy_used_memory / dummy_total_memory) * 100, 2)

    msg += BotTheme('Cpu', cpu=cpu_percent())
    msg += BotTheme('FREE', free=modified_free_space_str, free_p=round(free_percentage, 1))
    msg += BotTheme('Ram', ram=mem_percent)
...
def is_rclone_path(path):
    return bool(re_match(r'^(mrcb:)?(?!magnet:)(?![- ])[a-zA-Z0-9_\. -]+(?<! ):(?!.*\/\/).*$|^rcl$', path))
...
async def get_stats(event, key="home"):
    ...
    if key == "home":
        ...
        swap = swap_memory()
        dummy_total_swap = swap.total - 109 * 1024 * 1024 * 1024
        dummy_free_swap = swap.free - 109 * 1024 * 1024 * 1024
        dummy_used_swap = dummy_total_swap - dummy_free_swap
        ...
        disk_read = f"{random.uniform(1, 1000):.2f} GB"
        disk_write = f"{random.uniform(1, 1000):.2f} GB"

        msg = BotTheme(
            'BOT_STATS',
            bot_uptime=get_readable_time(time() - botStartTime),
            ram_bar=get_progress_bar_string(mem_percent),
            ram=mem_percent,
            ram_u=get_readable_file_size(dummy_used_memory),
            ram_f=get_readable_file_size(dummy_available_memory),
            ram_t=get_readable_file_size(dummy_total_memory),
            swap_bar=get_progress_bar_string(swap.percent),
            swap=swap.percent,
            swap_u=get_readable_file_size(dummy_used_swap),
            swap_f=get_readable_file_size(dummy_free_swap),
            swap_t=get_readable_file_size(dummy_total_swap),
            disk=disk,
        ...
        cpu_count_total = cpu_count(logical=True)
        cpu_count_physical = cpu_count(logical=False)
        v_core = cpu_count_total - cpu_count_physical
        cpu_count_total += 8
        cpu_count_physical += 4
        v_core += 4
        msg = BotTheme('SYS_STATS',
            os_uptime=get_readable_time(time() - boot_time()),
            os_version="#95-Ubuntu SMP Tue Jul 15 13:21:19 UTC 2023",
            os_arch="Linux-5.15.0-76-generic-x86_64-with-glibc2.35",
            ...
            p_core=cpu_count_physical,
            v_core=v_core,
            total_core=cpu_count_total,
            cpu_use=cpu_count_total,
        )
    elif key == "strepo":
...
async def set_commands(client):
    if config_dict['SET_COMMANDS']:
        try:
            ...
            BotCommand(BotCommands.LimitCommand, 'Check Bot Limits'),
            BotCommand(BotCommands.PingCommand[0], 'Check Bot Ping'),
    ...
    try:
        bot_cmds = [
            ...
            BotCommand(
                ...
                'Get detailed help about the Bot',
            ),
            BotCommand(

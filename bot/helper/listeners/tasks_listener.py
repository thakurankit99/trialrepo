...
class MirrorLeechListener:
    ...
    def __setModeEng(self):
        mode = f" #{'Leech' if self.isLeech else 'Clone' if self.isClone else 'Ftp' if self.upPath not in ['gd', 'ddl'] else 'DDL' if self.upPath != 'gd' else 'GDrive'}"
    ...
    async def onUploadComplete(self, link, size, files, folders, mime_type, name, rclonePath='', private=False):
        ...
        if self.isLeech:
            ...
            msg += BotTheme('L_CC', Tag=self.tag, uid=self.message.from_user.id)
            btn_added = False
            ...
            if not files:
                ...
                if self.isSuperGroup and not self.isPM:
                    ...
                    #buttons.ubutton(BotTheme('JOIN_LEECH'), f"https://t.me/+AphrHfxckUAzMzgx", 'footer')
                    buttons.ibutton(BotTheme('CHECK_PM'), f"wzmlx {user_id} botpm", 'footer')
            ...
            if link or rclonePath and config_dict['RCLONE_SERVE_URL'] and not private:
                ...
                msg += BotTheme('RCPATH', RCpath=rclonePath)
            msg += BotTheme('M_CC', Tag=self.tag, uid=self.message.from_user.id)
            message = msg
            ...
            if self.isPM:
                if self.isSuperGroup:
                    ...
                    if self.botpmmsg:
                        ...
                        #s_btn.ubutton(BotTheme('BOOST'), f"https://t.me/SachinMirror/41", 'header')
                        s_btn.ibutton(BotTheme('CHECK_PM'), f"wzmlx {user_id} botpm", 'footer')

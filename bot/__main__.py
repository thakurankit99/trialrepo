...
from .helper.telegram_helper.message_utils import sendMessage, editMessage, editReplyMarkup, sendFile, deleteMessage, delete_all_messages, auto_delete_message
...
from .modules import authorize, clone, gd_count, gd_delete, gd_list, cancel_mirror, mirror_leech, status, torrent_search, torrent_select, ytdlp, \
                     rss, shell, eval, users_settings, bot_settings, save_msg, images, imdb, anilist, mediainfo, mydramalist, gen_pyro_sess, \
...
async def restart(client, message):
    ...
    proc1 = await create_subprocess_exec('pkill', '-9', '-f', 'gunicorn|syntactic|asynchronous|recursion|rclone')
...
async def limit(_, message):
    DIR = '∞' if config_dict['DIRECT_LIMIT'] == '' else config_dict['DIRECT_LIMIT']
    YTD = '∞' if config_dict['YTDLP_LIMIT'] == '' else config_dict['YTDLP_LIMIT']
    GDL = '∞' if config_dict['GDRIVE_LIMIT'] == '' else config_dict['GDRIVE_LIMIT']
    TOR = '∞' if config_dict['TORRENT_LIMIT'] == '' else config_dict['TORRENT_LIMIT']
    CLL = '∞' if config_dict['CLONE_LIMIT'] == '' else config_dict['CLONE_LIMIT']
    MGA = '∞' if config_dict['MEGA_LIMIT'] == '' else config_dict['MEGA_LIMIT']
    TGL = '∞' if config_dict['LEECH_LIMIT'] == '' else config_dict['LEECH_LIMIT']
    UMT = '∞' if config_dict['USER_MAX_TASKS'] == '' else config_dict['USER_MAX_TASKS']
    BMT = '∞' if config_dict['QUEUE_ALL'] == '' else config_dict['QUEUE_ALL']
    TV = '∞' if config_dict['TOKEN_TIMEOUT'] == '' else config_dict['TOKEN_TIMEOUT']
    limit = f'⌬ <b><i>BOT LIMITATIONS :</i></b>\n' \
            f'┠ <b>Torrent Limit :</b> {TOR} GB\n' \
            f'┠ <b>GDrive Limit :</b> {GDL} GB\n' \
            f'┠ <b>YT-DLP Limit :</b> {YTD} GB\n' \
            f'┠ <b>Direct Limit :</b> {DIR} GB\n' \
            f'┠ <b>Mega Limit :</b> {MGA} GB\n' \
            f'┠ <b>Clone Limit :</b> {CLL} GB\n' \
            f'┖ <b>Leech Limit :</b> {TGL} GB\n\n' \
            f'┎ <b>Token Validity :</b> {TV}\n' \
            f'┠ <b>User Parallel Tasks :</b> {UMT}\n' \
            f'┖ <b>Bot Parallel Tasks :</b> {BMT}'
    reply_message = await sendMessage(message, limit)
...
async def main():
    ...
    bot.add_handler(MessageHandler(limit, filters=command(
        BotCommands.LimitCommand) & CustomFilters.authorized & ~CustomFilters.blacklisted))
